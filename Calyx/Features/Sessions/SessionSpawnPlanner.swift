...
struct SessionSpawnContext: Sendable, Equatable {
    ...
    init(cwd: String? = nil, inheritedCwd: String? = nil, name: String? = nil, origin: SessionSpawnOrigin = .tab) {
        self.cwd = cwd
...
enum SessionSpawnPlanner {
    ...
    /// `SessionSpawnOrigin`'s doc comment), or `resolver` can't find the
    /// calyx-session binary at all (a `nil` resolution must not fall
    /// back to a hardcoded `"calyx-session"` literal that assumes `PATH`
    /// availability — see `SessionBinaryResolver`'s doc comment).
    /// Otherwise generates a fresh ULID session ID and the matching
    /// attach command, using `inheritedCwd ?? cwd ?? home` as the
    /// effective working directory (a new split should land in the
